...
class Player:
    """Player character with physics body, double-jump, wall-slide, dash, and levitate abilities."""
    ...
    def take_damage(self, enemy=None):
        """Apply damage and knockback. Returns False if still invincible.

        Knockback direction is opposite to current movement; if standing still,
        opposite to the direction the enemy is relative to the player.
        """
        if self.invincibility_timer > 0:
            return False
        self.invincibility_timer = 2.0

        if abs(self.body.vx) > 50:
            knock_dir = -1 if self.body.vx > 0 else 1
        elif enemy is not None:
            knock_dir = 1 if self.body.x >= enemy.body.x else -1
        else:
            knock_dir = 1

        self.body.vx = knock_dir * 600
        self.body.vy = -400
        return True
