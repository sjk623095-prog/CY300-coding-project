...
class EnemyStateMachine:
    """Finite state machine for enemies; drives enter/update/exit lifecycle on each state change."""
...
class EnemyState:
    """Base class for all enemy states; subclasses override update() for behaviour logic."""
...
class PatrolState(EnemyState):
    """Enemy walks back and forth; switches to ChaseGroundState when the player enters detection range."""
...
class ChaseGroundState(EnemyState):
    """Ground enemy runs directly toward the player at chase speed."""
...
class HoverState(EnemyState):
    """Flying enemy bobs in place; switches to ChaseAirState when the player enters detection range."""
...
class ChaseAirState(EnemyState):
    """Flying enemy homes in on the player's position using normalized direction vectors."""
...
class EnemyBase:
    """Shared base for all enemies: physics body, ground/wall resolution, hazard and player contact checks."""
...
class PatrolEnemy(EnemyBase):
    """Ground enemy that patrols a platform and chases the player when spotted."""
...
class FlyingEnemy(EnemyBase):
    """Airborne enemy that hovers in place until it detects the player, then homes in freely."""
